...
def _build_event_handler_and_url_generator(state_class: type['ReactiveState'], original_func, event_config: dict):
    ...
    async def _handler(request):
        ...
        for name, param in list(sig.parameters.items())[1:]:  # Skip 'self'
            ...
            if raw is None and param.default is inspect.Parameter.empty:
                return f"Missing query param: {name}"
            if raw is not None:
                ann = typing.get_origin(param.annotation) or param.annotation
                if callable(ann) and ann in (int, float, bool):
                    try:
                        bound[name] = ann(raw)
                    except ValueError:
                        return f"Invalid type for query param: {name}, expected {ann.__name__}"
                else:
                    bound[name] = raw
...
def event(
    ...
    *,
    ...
    if isinstance(_func_or_pos_path, str):
        ...
        # Called as @event directly on a function (e.g., @event \n def func(...))
        if path is not None:
             raise ValueError(
